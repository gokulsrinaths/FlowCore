-- Pending invites are unique per (org, email) and (org, case, email) — see ux_invitations_pending_*.
-- Re-sending an invite should replace the old row instead of failing with unique_violation.
