-- Cases: remove mistaken "oblique" column; reference is district + crime number only.

DROP FUNCTION IF EXISTS public.flowcore_create_case(uuid, text, text, text, text, text, jsonb, numeric, text);
...
DROP FUNCTION IF EXISTS public.flowcore_update_case(uuid, uuid, text, text, text, text, text, jsonb, numeric, text);
...
ALTER TABLE public.cases DROP COLUMN IF EXISTS oblique;
